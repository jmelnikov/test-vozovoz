...
На основной странице форма, дублирующая функционал формы на главной странице сайта [Vozovoz](https://vozovoz.ru/).
На странице `/terminals` поиск терминалов по городам.

### Резюме проекта

1. **Интеграция с API**:
    - Проект интегрируется с API Vozovoz для получения городов, терминалов и расчета стоимости доставки.
    - Токен API настраивается в файле `app/config/params.php`.

2. **Функциональность фронтенда**:
    - Главная страница включает форму, имитирующую функциональность главной страницы сайта Vozovoz.
    - Форма позволяет пользователям вводить данные о доставке и рассчитывать стоимость доставки.
    - JavaScript используется для обработки ввода пользователя, получения данных из API и динамического обновления
      интерфейса.

3. **Кэширование**:
    - Redis используется для кэширования ответов API для улучшения производительности.
    - Время жизни кэша по умолчанию установлено на 1 час (3600 секунд), но может быть настроено для каждого сервиса.

4. **Настройка Docker**:
    - Проект включает конфигурации Docker и Docker Compose для легкой настройки и развертывания одной командой.

5. **Эндпоинты**:
    - `/api/cities`: Получает список городов на основе строки запроса.
    - `/api/terminals`: Получает список терминалов в указанном городе.
    - `/api/calc/form`: Рассчитывает стоимость доставки на основе предоставленных параметров.

6. **Обработка ошибок**:
    - Проект включает обработку ошибок для запросов к API, отображая соответствующие сообщения пользователю.
...
Для кэширования используется **Redis**, в качестве ключа используется **MD5 хэш** от сериализованного
представления параметров для поиска.
...
Если API токен в файле `app/config/params.php` недействителен, новый можно найти по адресу https://vozovoz.ru/dev/api/

#### Получение городов

На эндпоинт `/api/cities` отправить `GET` запрос со следующими параметрами:
...
На эндпоинт `/api/terminals` отправить `GET` запрос со следующими параметрами:
...
На эндпоинт `/api/calc/form` отправить `GET` запрос со следующими параметрами:
